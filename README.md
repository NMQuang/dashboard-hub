# dashboard-hub.hq — Personal Command Center
...
│   │   ├── connect/page.tsx      ← Check-in & messaging
│   │   ├── plan/page.tsx         ← Shared calendar
│   │   ├── finance/page.tsx      ← Budget tracking
│   │   └── tasks/page.tsx        ← Shared to-do list
│   ├── learn/
...
| `FAMILY_PASSWORD` | optional | Shared family password | Protect `/family` routes |
| `FAMILY_KV_REST_API_URL` | optional | Vercel KV / Upstash | Family data storage |
| `R2_ACCOUNT_ID` | optional | Cloudflare R2 | Family photos/videos |
...
FAMILY_COOKIE_SALT=random-string-123
FAMILY_KV_REST_API_URL=https://...
...
| `/family` | Family dashboard, timeline | KV |
| `/family/memories` | Upload photos, AI captions, face detection | R2 + KV + Claude |
| `/family/connect` | Daily check-in, messaging | KV |
| `/family/plan` | Shared calendar, events | KV |
| `/family/finance` | Budget tracking, VND/JPY coversion | KV + Market |
| `/family/tasks` | Shared to-do list | KV |
...
#### `family-hub` — Family features

**Kích hoạt khi:** hỏi về tính năng family, upload ảnh, r2, vercel kv, budget, tasks.

```
"Add a photo upload to /family/memories"
"Build a budget tracker in /family/finance"
"How do I use R2 to upload videos in Family hub?"
```

Skill cung cấp: R2 upload patterns, KV schema, layout components, authentication patterns cho family members.
