...
AI features (services/family-ai.ts):
  Caption           → Codex Vision → tiếng Việt
  Face detection    → Codex Vision → 'bé' | 'vợ' | 'chồng'
  Story generation  → Codex text → title + narrative
  JP translation    → Codex haiku → check-in tiếng Nhật → tiếng Việt
...
| `services/family-ai.ts` | Codex Vision: caption, faces, story, translate |
...
6. Background: Codex Vision generates caption + detects faces
